...
function parse(code, options) {
  ...
  let log = false;
  if (code.indexOf('createElement') !== -1) {
    log = true
  }
  const ast = parseCode(code);
  const imported = getImported(ast, log);
  const exported = getExported(ast, code);
